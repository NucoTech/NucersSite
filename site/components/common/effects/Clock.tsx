...
export default class Clock extends React.Component<ClockProps, ClockStates> {
    ...
    countTime = () => {
        ...
        document.getElementById("clock-hour").style.transform = `rotate(${
            ((hour - 12) / 12) * 360 - 90
        }deg)`
        document.getElementById("clock-minute").style.transform = `rotate(${
            (minute / 60) * 360 - 90
        }deg)`
        document.getElementById("clock-second").style.transform = `rotate(${
            (second / 60) * 360 - 90
        }deg)`
